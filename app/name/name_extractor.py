    ...
    async def _extract_name_with_llm(self, text: str, filename: str) -> Optional[str]:
        ...
        try:
            ...
            if use_openai() and getattr(settings, "openai_api_key", None) and str(settings.openai_api_key).strip():
                try:
                    raw_output = await openai_generate(
                        prompt=prompt,
                        system_prompt=NAME_PROMPT[:800],
                        temperature=0.0,
                        timeout_seconds=float(OLLAMA_TIMEOUT),
                    )
                except Exception as e:
                    # When LLM_MODEL=OpenAI we do NOT fallback to OLLAMA for resume parsing.
                    logger.error(
                        "OpenAI name extraction failed and OLLAMA fallback is disabled when LLM_MODEL=OpenAI",
                        extra={"file_name": filename, "error": str(e)},
                    )
                    raise
            ...
            if not raw_output or not raw_output.strip():
                ...
                if use_openai() and getattr(settings, "openai_api_key", None) and str(settings.openai_api_key).strip():
                    raise RuntimeError(
                        "OpenAI name extraction returned empty response and "
                        "OLLAMA fallback is disabled when LLM_MODEL=OpenAI"
                    )

                logger.debug(
                    f"📤 CALLING OLLAMA API for name extraction (timeout: {OLLAMA_TIMEOUT}s)",
                    extra={"file_name": filename, "model": model_to_use, "ollama_host": self.ollama_host, "text_length": len(text)}
                )
                result = None
                last_error = None
                        ...
                        try:
                            response = await client.post(
                                ...
                                json={
                                    ...
                                    "stream": False,
                                    "options": {"temperature": 0.0, "top_p": 0.9, "num_predict": OLLAMA_MAX_TOKENS}
                                }
                            )
                            response.raise_for_status()
                            result = response.json()
