...
Output (JSON only, no other text, no explanations):"""
            ...
            if use_openai() and getattr(settings, "openai_api_key", None) and str(settings.openai_api_key).strip():
                try:
                    logger.info("Using OpenAI for role extraction", extra={"file_name": filename})
                    raw_output = await openai_generate(
                        prompt=prompt,
                        system_prompt=ROLE_PROMPT[:500],
                        temperature=0.1,
                    ...
                    logger.error(
                        "OpenAI role extraction failed and OLLAMA fallback is disabled when LLM_MODEL=OpenAI",
            ...
            if not raw_output or not raw_output.strip():
                ...
                if use_openai() and getattr(settings, "openai_api_key", None) and str(settings.openai_api_key).strip():
                    raise RuntimeError(
                        "OpenAI role extraction returned empty response and "
                ...
                async with httpx.AsyncClient(timeout=Timeout(600.0)) as client:
                    try:
                        ...
                        logger.info("📥 OLLAMA API RESPONSE RECEIVED", extra={"response_text_length": len(response_text)})
