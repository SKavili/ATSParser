...
Output (JSON only, no other text, no explanations):"""
            ...
            if use_openai() and getattr(settings, "openai_api_key", None) and str(settings.openai_api_key).strip():
                try:
                    logger.info("Using OpenAI for education extraction", extra={"file_name": filename})
                    raw_output = await openai_generate(
                        prompt=prompt,
                        system_prompt=EDUCATION_PROMPT[:800],
                        temperature=0.1,
                    ...
                    logger.error(
                        "OpenAI education extraction failed and OLLAMA fallback is disabled when LLM_MODEL=OpenAI",
            ...
            if not raw_output or not raw_output.strip():
                ...
                if use_openai() and getattr(settings, "openai_api_key", None) and str(settings.openai_api_key).strip():
                    raise RuntimeError(
                        "OpenAI education extraction returned empty response and "
