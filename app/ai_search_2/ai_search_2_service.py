...
from app.ai_search_2.ai_search_2_designation_matcher import DesignationMatcher
...
class AISearch2Service:
    ...
    # ai-search-2: keep role matching query-driven (LLM/query text) rather than static maps.
    # These are intentionally empty to disable static role-family behavior in this service.
    ...
    async def search_semantic(
        ...
        try:
            ...
            # ai-search-2: do not apply static hard post-filters in service.
            # Final role precision is handled in controller with query-driven logic.
    ...
    def _get_smart_namespaces(self, parsed_query: Dict) -> List[tuple]:
        ...
        ai-search-2 requirement:
        - Query across ALL available IT and NON_IT namespaces.
        """
        ...
        logger.info(
            f"All-namespace mode: querying {len(deduped)} namespaces (all IT + all NON_IT)",
            extra={"namespace_count": len(deduped)}
        )
    ...
    async def _query_namespaces_parallel(
        ...
        # No split-by-namespace-count limit, but keep a safe per-namespace cap
        # to avoid timeout storms when querying all namespaces.
        has_designation = bool((parsed_query or {}).get("filters", {}).get("designation"))
        per_namespace_cap = int(getattr(settings, "ai_search_2_top_k_per_namespace_cap", 10))
        top_k_per_namespace = max(1, min(top_k, per_namespace_cap))

        logger.info(
            "Broad namespace fanout config",
            extra={
        ...
        # Create tasks for parallel execution and collect partial results by timeout.
        task_to_namespace = {}
        tasks = []
        for mastercategory, namespace in namespaces:
            task = asyncio.create_task(
                self.pinecone_automation.query_vectors(
                    query_vector=embedding,
                    mastercategory=mastercategory,
                    namespace=namespace,
            ...
            tasks.append(task)
            task_to_namespace[task] = (mastercategory, namespace)

        all_results: List[Dict[str, Any]] = []
        done, pending = await asyncio.wait(tasks, timeout=timeout_seconds)

        for task in done:
            namespace_info = task_to_namespace.get(task, ("unknown", "unknown"))
            try:
                result = task.result()
                ...
                logger.warning(
                    f"Namespace query failed: {e}",
                    extra={"namespace": namespace_info, "error": str(e)},
                )
        ...
        if pending:
            ...
            logger.warning(
                ...
                extra={
                    ...
                    "completed_namespace_queries": len(done),
                    "cancelled_namespace_queries": len(pending),
                    "namespaces_queried": len(namespaces),
                },
            )
